...
// 使用异常式断言，使失败信息能直接指出哪条智能默认规则不符合预期。
...
int main() {
    ...
    // 每次测试创建独立临时目录，避免并行运行或上次残留造成路径冲突。
    ...
    try {
        ...
        // 场景四：SBA 文件应生成解包目录，且不能被误用作 Pack 的源目录。
