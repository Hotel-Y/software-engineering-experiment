...
bool isSameOrInside(const std::filesystem::path& child, const std::filesystem::path& parent) {
...
OperationStats BackupManager::backup(const std::filesystem::path& sourceDir,
    ...
    if (isSameOrInside(backupDir, sourceDir)) {
    ...
    for (const auto& item : std::filesystem::recursive_directory_iterator(
        ...
        if (!shouldIncludeFile(item.path(), fileSize)) {
            ++stats.skippedFiles;
            continue;
        }
...
OperationStats BackupManager::restore(const std::filesystem::path& backupDir,
    ...
    for (const auto& entry : manifest.entries()) {
        const auto relativePath = pathFromUtf8(entry.relativePath);
        const auto sourcePath = backupDir / relativePath;
        const auto targetPath = restoreDir / relativePath;
...
OperationStats BackupManager::verify(const std::filesystem::path& backupDir) const {
    ...
    for (const auto& entry : manifest.entries()) {
        ...
        const auto filePath = backupDir / pathFromUtf8(entry.relativePath);
